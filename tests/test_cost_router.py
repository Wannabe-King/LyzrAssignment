...
def test_complexity_heuristic():
    ...
    assert estimate_complexity("analyze this trace") == 13  # 3 words + 10
    assert estimate_complexity("Compare and analyze the architecture") == 35  # 5 + 30
...
def test_budget_enforced_before_call():
    ...
    with pytest.raises(BudgetExceeded):
        r.run_task("second")
    assert m["small"]["client"].call_count == 1  # second call never made


def test_escalation_respects_budget():
    m = models([UNSURE], [SURE])
    r = CostRouter(m, budget=5)  # can afford small (1) but not large (10)
    result = r.run_task("short")
    assert result["model"] == "small"  # kept cheap answer instead of busting budget
