...
def test_regenerates_with_critique_constraint():
    ...
    assert "too vague, add numbers" in retry_prompt  # critique fed back
    assert "bad draft" in retry_prompt               # previous output fed back
...
def test_stops_at_max_attempts_returns_best():
    ...
    assert result["output"] == "d2" and result["score"] == 0.6  # best, not last
    assert abs(result["improvement"] - 0.2) < 1e-9  # last - first
