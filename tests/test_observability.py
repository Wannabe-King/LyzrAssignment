...
class FakeClock:
    ...
    def __call__(self):
        self.t += 1.0  # each clock() read advances 1s
        return self.t
...
def test_span_duration_and_order():
    ...
    assert s["duration"] == 1.0  # entry=1, exit=2
...
def test_nested_spans_have_parents():
    ...
    assert tracer.spans[0]["name"] == "inner"  # finish order
...
def test_loop_alarm_fires_at_threshold_multiples():
    ...
    assert len(llm.alerts) == 2  # at 3rd and 6th repetition
