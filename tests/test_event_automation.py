...
def flaky(fail_times):
    """Handler that raises `fail_times` times, then succeeds."""
...
def test_retry_with_backoff_then_success():
    ...
    assert sleeps == [1, 2]  # 2**0, 2**1 between attempts
...
def test_never_raises():
    ...
    p.process(ev(1))  # must not raise


def test_replay_dead_letter():
    handler = flaky(4)  # fails 4 times total; with 0 retries: 4 DLQ rounds... use 2 retries
    sleeps = []
    p = EventProcessor({"job": handler}, max_retries=2, sleep=sleeps.append)
    p.process(ev(1))  # attempts 1-3 fail -> dead letter
    assert p.dead_letter
    recovered = p.replay_dead_letter()  # attempt 4 fails, attempt 5 succeeds
    assert recovered == 1
    assert p.dead_letter == []
